---
layout: page
title: Dimensionality Reduction and Transforms (SURF 2023)
description: Summer Undergraduate Research Fellowship 2023
img: assets/img/surf-2023-thumb.jpg
importance: 5
category: work
related_publications: false
---

{% assign poster_pdf = site.static_files | where: "path", "/assets/surf/2023-dimred.pdf" | first %}
{% assign poster_png = site.static_files | where: "path", "/assets/surf/2023-dimred.png" | first %}
{% assign poster_jpg = site.static_files | where: "path", "/assets/surf/2023-dimred.jpg" | first %}
{% if poster_pdf %}<p><a href="/assets/surf/2023-dimred.pdf">Download poster (PDF)</a></p>{% endif %}
{% if poster_png %}<p><img src="/assets/surf/2023-dimred.png" alt="SURF 2023 poster" style="width:70%;height:auto;"></p>{% endif %}
{% if poster_jpg %}<p><img src="/assets/surf/2023-dimred.jpg" alt="SURF 2023 poster" style="width:70%;height:auto;"></p>{% endif %}
